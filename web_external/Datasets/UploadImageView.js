...
import UploadImageTemplate from './uploadImage.pug';
import './uploadImage.styl';
import './uploadWidget.styl';

const UploadImageView = View.extend({
    events: {
        'click #isic-upload-reset': function (event) {
            this.resetUpload();
        },
        'click #isic-upload-image-submit': function (event) {
            ...
            if (uploadedFileId) {
                this.$('#isic-upload-image-submit').girderEnable(false);
                // TODO
            } else {
                showAlertDialog({ text: 'Please upload an image.' });
            }
        }
    },
    ...
    render: function () {
        this.$el.html(UploadImageTemplate({
    ...
    uploadStarted: function (files) {
        ...
        if (this.uploadFolder) {
            ...
            this.uploadFolder = new FolderModel({
                name: `isic_image_upload_${Date.now()}`,
                parentType: 'user',
                parentId: getCurrentUser().id,
                description: 'ISIC image upload'
            });
